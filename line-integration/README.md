...
OpusBoosterにログインしたら、画面上部のナビメニューにある「**LINE連携**」アイコン（吹き出しのマーク）をクリックすると、この画面に入れます。

<figure><img src="../.gitbook/assets/line-overview-entry.png" alt=""><figcaption>OpusBoosterの上部ナビメニューにある「LINE連携」アイコンから入ります</figcaption></figure>

友だちから新しいメッセージが届くと、このアイコンに赤い数字が出ます。まだ誰も開いていない会話の人数です。数字の見方と既読になるタイミングは、[トークで返信する](watch/talk.md)の「未読を確かめる」で説明しています。

### 初めて開くときは、メールでログインします

「LINE連携」を初めて開くと、メールアドレスの入力を求められます。OpusBoosterに登録したのと同じメールアドレスを入れて「ログイン用のリンクを送る」を押すと、ログイン用のリンクがメールで届きます。そのリンクを開くと、LINE連携の画面に入れます。

<figure><img src="../.gitbook/assets/line-login-magic-link.png" alt=""><figcaption>メールアドレスを入れて「ログイン用のリンクを送る」を押すと、ログイン用のリンクがメールで届きます</figcaption></figure>

リンクは、送信したのと同じブラウザ（同じウィンドウ）で開いてください。別のブラウザで開くと、ログインが完了しません。

メールが見当たらないときは、迷惑メールのフォルダも確かめてください。
