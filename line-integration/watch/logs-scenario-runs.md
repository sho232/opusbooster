...
気になる行の「**詳細**」を押すと「**ステップ実行履歴**」が開きます。ここでは「**ステップ**」「**ステータス**」「**待機結果**」「**開始日時**」「**終了日時**」「**エラー**」「**再試行回数**」を確認できます。終えたら「**閉じる**」で一覧へ戻せます。

<figure><img src="../../.gitbook/assets/line-scenario-runs-detail.png" alt=""><figcaption>どのステップが成功・失敗・スキップしたかを確認します</figcaption></figure>

「**条件が満たされるまで待つ**」のステップでは、「**待機結果**」に次のどれかが表示されます。

| 待機結果 | 意味 |
|---|---|
| **条件を満たして次へ** | 待っているあいだに条件のタグが付いた（または外れた）ので、次のステップへ進みました |
| **入った時点で満たしていた** | このステップに入った時点で、すでに条件を満たしていました。待たずに次へ進んでいます |
| **期限切れ** | 最長で待つ期間を過ぎました。「**期限を過ぎたら**」の設定どおりに進んだか、シナリオを終えています |

まだ待っているあいだは、そのステップの状態が「**実行中**」のまま表示されます。
