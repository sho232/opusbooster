...
ビルダーでは流れの中にステップを追加します。最初は「**メッセージ送信**」と「**待機**」を組み合わせると分かりやすいでしょう。「**メッセージ送信**」には友だちへ届ける本文を入れます。「**待機**」を間に置けば、次の案内までの間隔を作れます。最初にあいさつを送り、待ち時間のあとに試聴やライブ情報を案内する形です。ステップは上から順に実行されます。

<figure><img src="../../.gitbook/assets/line-scenario-builder-steps.png" alt=""><figcaption>メッセージ送信と待機を順番に置きます</figcaption></figure>

## 条件が満たされるまで待つ

「**待機**」は決めた時間だけ待ちますが、「**条件が満たされるまで待つ**」は、**友だちにタグが付く（または外れる）まで待ってから**次のステップへ進みます。たとえば、下の「**リンククリックでタグを付ける**」で案内のリンクを開いた人に付くタグを条件にすれば、「リンクを開いてくれた人にだけ、次の案内を送る」流れを作れます。

ステップの追加で「**待機**」のすぐ下にある「**条件が満たされるまで待つ**」を選び、次の 3 つを決めます。

| 項目 | 決めること |
|---|---|
| **条件** | どのタグを見るか。「**タグを持つ**」「**タグを持たない**」を選び、タグを選びます。「**条件を追加**」で最大 10 件まで並べられ、「**すべて満たす**」か「**いずれか満たす**」かを選べます |
| **最長で待つ期間** | 条件を満たさないまま待つ上限。1 時間〜30 日の間で、時間または日で決めます（最初は 3 日） |
| **期限を過ぎたら** | 上限まで待っても満たさなかった人をどうするか。「**次のステップへ進む**」か「**このシナリオを終える**」を選びます |

<figure><img src="../../.gitbook/assets/line-scenario-wait-until-form.png" alt=""><figcaption>条件・最長で待つ期間・期限を過ぎたときの動きを決めます</figcaption></figure>

ビルダーには「**タグ『〇〇』が付くまで待つ (最長 3 日)**」のように、条件と期限が 1 行で表示されます。いま何人がこのステップで待っているかも、ステップの下の「**待機**」の人数で分かります。

仕組みとして、次の点を知っておいてください。

- **このステップに入った時点で条件を満たしている人は、待たずに次へ進みます。** 前に付いたタグが残っていると、すぐに通過します。
- タグが付いてから次のステップへ進むまで、**少し時間がかかることがあります**。
- 期限は、**このステップに入った時点から**数えます。途中でシナリオを Off にしていた時間も含めて数えます。Off のあいだに期限を過ぎた人は、On に戻したときに条件を確かめ、満たしていれば次へ進み、満たしていなければ期限切れとして扱われます。
- 待っているあいだに条件のタグを変えた場合は、**保存した時点から新しい条件で判定します**。すでに新しい条件を満たしている人は、そのまま次へ進みます。最長で待つ期間の変更は、**これから入る人から**効きます。
- シナリオを**アーカイブすると、このステップで待っていた人の配信は止まります**。下書きに戻しても、止まった人は再開しません。
- **目標**（買ってもらえたら止める など）に当てはまった人は、このステップを抜けるときに止まります。

### 期限を過ぎた人を別の案内に分けたいとき

「**期限を過ぎたら**」で「**次のステップへ進む**」を選び、このステップの**すぐ後ろに「条件で分岐」**を置いて、同じタグで分けてください。条件を満たして進んできた人と、期限を過ぎて進んできた人を、別々のメッセージに分けられます。

{% hint style="info" %}
「**このシナリオを終える**」を選ぶと、期限を過ぎた人はそこで終わり、後ろのステップは実行されません。「条件を満たした人にだけ、次のステップ（タグ付けなど）を実行したい」ときに使えます。
{% endhint %}

{% hint style="warning" %}
**AI が作るシナリオでは、このステップは使われません。** 必要な場合は、作られたシナリオにご自分で追加してください。
{% endhint %}
...
「**条件が満たされるまで待つ**」を含むシナリオも共有できます。条件に使っているタグは、取り込み先の同じタグにつながります。取り込み先でそのタグがアーカイブ済みの場合は、取り込めません（「**条件待ちステップで使うタグがアーカイブ済みです。タグを復元してから、もう一度インポートしてください。**」と表示されます）。タグ管理でタグを復元してから、もう一度取り込んでください。
...
### 「条件が満たされるまで待つ」で、待たずに次へ進んでしまいました

このステップに入った時点で、すでに条件を満たしていた可能性があります。前に付いたタグが残っていると、待たずに通過します。「**ログ**」の「**シナリオ実行ログ**」で、その方の「**詳細**」を開くと、「**待機結果**」に「**入った時点で満たしていた**」と表示されます。

### 「条件が満たされるまで待つ」で止まったままの人がいます

条件のタグがまだ付いていない（または外れていない）ためです。「**最長で待つ期間**」を過ぎると、「**期限を過ぎたら**」の設定どおりに進むか終わります。条件に使っているタグをアーカイブすると、そのタグはもう付かないため、期限まで待つことになります。

### 「条件が満たされるまで待つ」のステップを削除できません

このステップ（または一緒に削除される分岐先のステップ）で、いま待っている・実行中の方がいると削除できません。画面に表示される人数を確かめ、その方たちが先へ進むか止まってから、もう一度お試しください。
