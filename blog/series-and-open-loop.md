...
空欄のまま生成すると、AIが本文の流れを読んで自動で予告テーマを決めてくれます。1本ずつ書いていく場合は、空欄でも大丈夫です。

ただし、**3本の構成を先に決めてから書き始める場合は、空欄にせずご自分で書いてください**。空欄だとAIが決めたテーマが次回の出発点になり、あらかじめ立てた計画とずれることがあります。構成を先に決める方法は[シリーズ記事3本の設計シート](series-worksheet.md)にまとめています。

{% hint style="info" %}
次回予告は「テキストだけ」の予告として本文末に自動追加されます。次の記事へのリンクは張られません（次の記事はまだ存在しないので当然です）。あくまで、読者に「気になる次回の話がある」ことを伝える役割です。なお逆方向のリンク（続き記事から前回記事へのリンク）は自動で入ります。詳しくはパーツ③をご覧ください。
{% endhint %}
...
続き記事の冒頭には「前回は〜」という振り返りの一文が入り、その言及には**前回記事へのリンクが自動で張られます**。読者が前の話をすぐ読み返せるだけでなく、記事同士の内部リンクとしてSEOにも役立ちます。

{% hint style="info" %}
前回記事へのリンクが自動で入るのは、前回の記事を最近のアップデート以降に保存している場合です。それより前に公開したシリーズ記事の続きを書くと、冒頭の言及はリンクなしのテキストになります。その場合は、前回の記事を一度開いて保存し直してください。以降に書く続き記事からはリンクが入るようになります。
{% endhint %}
...
**Q: 前回の記事へのリンクは入りますか？**
A: 入ります。シリーズの続きとして生成した記事は、冒頭の「前回の振り返り」が前回記事へのリンクになります。リンクが入らないときは、前回の記事を一度開いて保存し直してから続きを書いてください（パーツ③の補足をご覧ください）。
