...
**予約や注文の完了メールも、この設定を待たずに届きます。** これらの「システムメール」は、設定が終わるまでは OpusBooster の共通アドレス（`no-reply@opusbooster.com` から始まる差出人）で送られます。この設定が必要になるのは、**キャンペーンとオートメーションのメール**を自分のドメインから送るときです。
{% endhint %}
