...
各タブには、タイトルやアイコン、バッジ（「新着」「人気」などの小さなラベル）を追加でき、ホバー時のスタイルも調整できます。タブの並び順はドラッグで入れ替えられます。料金プランの比較や、サービス内容をカテゴリ別に見せたいときに向いています。

### 背景パターン

コンテナの背景に、ドットや線などの装飾パターンを追加できます。
...
コンテナ（行・カラム）を[スライダーにする](../website-funnel-builder/layout-freedom.md)と、以下の細かい表示設定が行えます。

* 矢印の表示・非表示、位置（サイドなど）
* バレット（インジケーター）の表示・非表示
* アニメーションの種類（スライドなど）
* オートスライド（自動再生）の有効化
* トランジションのイージング（動きの緩急。スムーズなど）
* パララックス スライドコンテンツ（背景と前景で動く速度を変える視差効果）
* キーボードナビゲーション（矢印キーでのスライド操作）
* タブが非表示・非フォーカス時に自動再生を一時停止

<figure><img src="../../.gitbook/assets/container-widget-slider-settings.png" alt=""><figcaption></figcaption></figure>

{% hint style="info" %}
基本的な「行やカラムをスライダーにする」手順は[並べ方が自由になりました](../website-funnel-builder/layout-freedom.md)を参照してください。ここで紹介しているのは、スライダーにした後の詳細な見た目・動きの調整項目です。
{% endhint %}

### 表示条件(コンテナの表示・非表示を条件で切り替える)

コンテナごとに、「誰に見せるか」を条件で設定できます。ブロック設定の中の「**表示条件**」から、以下を選べます。複数の条件を組み合わせることも可能です。
...
これを使うと、1つのページの中に「無料で読める部分」と「ここから先は会員限定」を同居させることができます。noteのような有料記事の形にしたい場合は、[メンバーシップ商品](../crm/membership-groups.md)を買い切りタイプで作成して特定の会員グループを付与し、記事の続き部分のコンテナに「特定の会員グループ限定」の表示条件を設定すれば、購入した人にだけ続きが表示される仕組みになります。

「ログイン会員限定」は、無料登録さえしてもらえれば見られる設定なので、メールアドレスやLINE登録と引き換えに続きを見せる、リスト取りの導線にも使えます。ページ全体を丸ごと隠したい場合は[メンバーシップ限定コンテンツの設定](../crm/membership-content.md)、ページの一部だけを出し分けたい場合はこちらの表示条件、と使い分けてください。
{% endhint %}
