...
* 自動再生の進捗バーの表示
* モバイル表示時は自動再生を無効にする
* タブが非表示の間は自動再生を一時停止する
* パララックス（背景と前景で動く速度を変える視差効果）
* キーボードでのスライド操作（矢印キー）
...
<figure><img src="../../.gitbook/assets/container-widget-visibility-conditions.png" alt=""><figcaption></figcaption></figure>

{% hint style="success" %}
**自分の場合はどう使えばいいか**

これを使うと、1つのページの中に「無料で読める部分」と「ここから先は会員限定」を同居させることができます。noteのような有料記事の形にしたい場合は、[メンバーシップ商品](../crm/membership-groups.md)を買い切りタイプで作成して特定のメンバーシップグループを付与し、記事の続き部分のコンテナに「特定のメンバーシップグループ」の表示条件を設定すれば、購入した人にだけ続きが表示される仕組みになります。

「ログイン済みメンバー」は、無料登録さえしてもらえれば見られる設定なので、メールアドレスやLINE登録と引き換えに続きを見せる、リスト取りの導線にも使えます。ページ全体を丸ごと隠したい場合は[メンバーシップ限定コンテンツの設定](../crm/membership-content.md)、ページの一部だけを出し分けたい場合はこちらの表示条件、と使い分けてください。
{% endhint %}
